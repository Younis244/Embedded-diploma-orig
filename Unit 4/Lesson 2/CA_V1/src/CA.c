...
State_Define (CA_Driving)
	{
		...
		//DC_motor(CA_speed)
		//Event check
		//US_get_ditance(CA_distance)//en heya betet check el distance 3ashan teddy actions
			CA_distance = US_get_distance_random(45 , 55 , 1);//Range w 1 de ely howa value w7da
			(CA_distance <= CA_threshold)? (CA_State = State(CA_Waiting) ) : (CA_State = State(CA_Driving) );
